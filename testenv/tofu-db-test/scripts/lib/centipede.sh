...
# the plan, not the migration. That is why tofu/{my,pg}-target runs first.
#
# ── Scope and rollback ──────────────────────────────────────────────────────
...
# ---------------------------------------------------------------------------
# The target connection — an inline db connection written into the request
# ---------------------------------------------------------------------------
# It is not registered with honeybee. A managed instance is created per column
# and gone when the column ends, so registering it would add one more thing to
# clean up.
#
...
cp_dst_connection() {
	...
	jq -n \
		--arg prov "$(lower "$CSP")" --arg t "$(lower "$engine")" \
		--arg h "$RDBMS_HOST" --argjson p "$RDBMS_PORT" \
		--arg u "$RDBMS_USER" --arg w "$(vault_db_password "$(lower "$CSP")")" \
		--arg tls "$tls" --arg auth "${MONGODB_AUTH_SOURCE:-admin}" \
		'{source:"db", db:(
		    {providerName:$prov, dbmsType:$t, accessType:"direct",
		     host:$h, port:$p, username:$u, password:$w}
		    + (if $tls == "disable" then {} else {tlsMode:$tls} end)
		    + (if $t == "mongodb" then {authSource:$auth} else {} end))}'
}
