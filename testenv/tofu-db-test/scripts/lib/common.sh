...
# to use one value: the target-database modules and privilege query in tofu.sh,
# the db.tlsMode centipede.sh puts in its request, and db-matrix.sh's pre-flight
...
# itself, not a path to it). Both are masked before anything reaches the screen
# or the file.
