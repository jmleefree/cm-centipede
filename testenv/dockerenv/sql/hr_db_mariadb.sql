...
CREATE OR REPLACE FUNCTION fn_performance_rating_text(p_score TINYINT)
RETURNS VARCHAR(20)
DETERMINISTIC
BEGIN
    RETURN CASE p_score
...
CREATE OR REPLACE FUNCTION fn_annual_salary(p_monthly_salary DECIMAL(12,2))
...
CREATE OR REPLACE PROCEDURE sp_dept_salary_report(IN p_dept_id INT)
BEGIN
    SELECT
...
CREATE OR REPLACE TRIGGER trg_after_employee_salary_update
...
CREATE OR REPLACE TRIGGER trg_before_review_insert
...
CREATE OR REPLACE TRIGGER trg_after_leave_status_update
...
CREATE OR REPLACE TRIGGER trg_after_employee_resign
AFTER UPDATE ON employees
FOR EACH ROW
BEGIN
...
CREATE OR REPLACE EVENT evt_archive_old_salary_history
...
CREATE OR REPLACE EVENT evt_auto_reject_stale_leaves
