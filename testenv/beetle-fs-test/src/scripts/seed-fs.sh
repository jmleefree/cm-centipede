#!/usr/bin/env bash
#
# seed-fs.sh — build the source dataset under $BASE.
#
# This is dockerenv's scripts/03-setup-filesystem.sh, file for file, so a result
# here is comparable with a docker-to-docker run. Only the first two lines differ:
# the path comes from FS_SRC_PATH instead of being hard-coded, because the matrix
# hands it in through /opt/matrix/matrix.env.
#
# 54 files, about 2.7 MB, 18 directories three levels deep. Text and binary
# mixed, and three files carry multibyte content or multibyte NAMES
# (documents/한글_인코딩_검증.txt, documents/日本語_エンコード検証.txt,
# data/json/metadata/utf8_encoding_check.json). Every other file is ASCII on
# purpose, so a transfer that mangles an encoding shows up in those three and
# nowhere else.
#
# ⚠ Keep this in step with dockerenv's copy. The point of it being identical is
#   that the two environments can be compared; a change here that is not made
#   there silently removes that.

set -euo pipefail

BASE="${FS_SRC_PATH:-/testdata}"
