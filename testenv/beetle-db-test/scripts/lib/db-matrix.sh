...
#
#   engine
#     └ one target version ─── cm-beetle creates the managed instance (5-30 min)
#          └ source versions ── cell: source container -> target DB -> honeybee -> centipede -> validation
#        ─── cm-beetle deletes it
#
...
Options:
  ...
  --target-db-create WHO       who creates the target database (default: $TARGET_DB_CREATE)
                               centipede — centipede creates it and owns it, so a
                                           failed migration drops it whole
                               matrix    — this script creates it first, so
                                           centipede finds it and only empties it
  --cleanup                    do not run the matrix; reclaim everything this
                               matrix left in the cb-tumblebug namespace
  -h, --help                   this help

⚠ Requires cm-honeybee, cm-centipede, cm-beetle and cb-tumblebug to be running,
  with the <csp>-<region> connection registered. Checked in steps 1 and 2.
...
Options that do not exist, and why:
  ...
  --secure-transport     there is no parameter-group API anywhere in the beetle
                         stack, so the CSP's setting cannot be changed from here.

One managed instance is created per target version, and creating one takes 5-30
minutes. If an interrupted run left resources behind, --cleanup reclaims them by
listing the namespace - there is no local state file to lose.
USAGE
}
...
parse_args() {
	while [ $# -gt 0 ]; do
		...
		--target-db-create) TARGET_DB_CREATE="$2"; shift 2 ;;
		...
		--secure-transport)
			die "--secure-transport does not exist on the beetle path — there is no parameter-group
       API in the stack, so the CSP's require_secure_transport cannot be changed. See --help." ;;
...
# assert_target_db_create — the value is legal, before anything is created.
#
#   It also normalises. matrix_defaults lowers what .env supplied, but parse_args
#   runs after it, so --target-db-create MATRIX would reach the cells as written
#   and fail every later comparison against "matrix" - silently taking the other
#   mode rather than erroring. Lowering here, after both, is the one place that
#   sees the final value.
assert_target_db_create() {
	TARGET_DB_CREATE="$(lower "${TARGET_DB_CREATE:-}")"
	case "$TARGET_DB_CREATE" in
	centipede|matrix) return 0 ;;
	esac
	fail "unknown TARGET_DB_CREATE value: '$TARGET_DB_CREATE'"
	fail "  Allowed values: centipede matrix"
	return 1
}

# assert_managed_engines ENGINES — engines centipede will accept behind a
#   beetleDb target reference, before anything is created.
#
#   This folder names its target as a beetleDb reference, and centipede's
#   validateManagedEngine (core/migration/dbms.go) rejects any engine but mysql
#   and mariadb there - cb-tumblebug provisions no other kind. That is the same
#   set beetle_engines already allows, so this refuses nothing that would
#   otherwise have run; it exists so the day BEETLE_ENGINES_<CSP> is widened, the
#   second wall is named here rather than found at plan time.
assert_managed_engines() {
	local engines="$1" engine
	for engine in $engines; do
		case "$(lower "$engine")" in
		mysql|mariadb) continue ;;
		esac
		fail "$engine cannot be a target behind a beetleDb connection."
		fail "  cm-centipede's validateManagedEngine accepts mysql and mariadb only there,"
		fail "  because cb-tumblebug provisions no other managed engine. This folder always"
		fail "  names its target as a beetleDb reference, so there is no inline-db path to"
		fail "  fall back to."
		fail "  If cm-beetle has since gained this engine, widen validateManagedEngine in"
		fail "  cm-centipede (pkg/core/migration/dbms.go) as well as BEETLE_ENGINES_$(upper "$CSP")."
		return 1
	done
	return 0
}
...
#   under either TARGET_DB_CREATE: centipede drops only what it created and only
#   when the migration failed, so a cell that passed always leaves one behind, and
#   the next cell in this column needs the name free and empty. Dropping one that
#   centipede already dropped is not an error - logical_db_drop treats absent as
#   done.
cleanup_cell() {
	local engine="$1" rc="$2"
...
run_cell() {
	...
	# ── 2) Target database ──────────────────────────────────────────────────
	# Who creates it is TARGET_DB_CREATE's whole job, and it decides more than it
	# looks: centipede's EnsureTargetDatabase reports whether it created the
	# database, and that flag is what picks the undo path for a failed cell - a
	# database centipede made is dropped whole, one it found is emptied by
	# transx-ex's rollback and kept.
	#
	# Either way anything left from a previous cell goes first. transx-ex requires
	# the target to exist and be empty, so a leftover fails as target-not-empty
	# under "matrix", and under "centipede" it also makes centipede find a database
	# it does not own - taking the wrong undo path for the rest of the cell.
	#
	# ACTIVE_TARGET_DB is set in both modes: it names what cleanup_cell has to
	# reclaim, not what this script created.
	if [ "$TARGET_DB_CREATE" = "matrix" ]; then
		sub "2) creating the target database — $ACTIVE_INSTANCE (this script)"
		...
		sub "2) target database — $ACTIVE_INSTANCE (left to cm-centipede)"
		target_db_clear "$CSP" "$engine" "$DST_DB"
		info "target database: $DST_DB will be created by cm-centipede through cm-beetle"
	fi
	...
	if ! cp_plan "$src_model" "$engine" "$SRC_DB" "$DST_DB"; then
		...
		if [ "$CP_DOWNGRADE" = "1" ] && [ "$expect_block" = "1" ]; then
			...
			explain_centipede_beetle "$CP_ERROR"
			explain_dbtype_mismatch "$CP_ERROR" "$engine"
	...
	# ── 5) Migration ────────────────────────────────────────────────────────
	mig_name="${MIGRATION_PREFIX:-cpbdb}-$engine-${sver//./_}-${dver//./_}-$(date '+%Y%m%d-%H%M%S')"
...
wait_target_reachable() {
	...
	fail "  Check public access (publicAccess=$RDBMS_PUBLIC) and the inbound rule on"
	fail "  security group $(sg_name "$CSP") ($MATRIX_ALLOWED_CIDR)."
	if [ "$(lower "$CSP")" = "ncp" ]; then
		fail "  On NCP, check the public domain has been issued too. publicAccess is an"
		fail "  observation, not an echo of the request: cb-spider sets it true only when the"
		fail "  instance actually has a public domain, and falls back to the private one"
		fail "  otherwise (ncp/resources/RDBMSHandler.go). false here means $RDBMS_HOST is"
		fail "  the private domain, which is unreachable from outside the VPC."
	fi
	return 1
}
...
print_matrix() {
	...
	banner "$CSP_TITLE version matrix result — source $MODE -> target direct  (beetle + honeybee + centipede)$cond"
...
write_result_json() {
	...
	jq -n --arg csp "$CSP" --arg prefix "$MATRIX_NAME_PREFIX" --arg ns "$MATRIX_NS" \
		--arg sm "$MODE" --arg ts "$(date '+%F %T')" --argjson e "$engines" \
		--arg gp "$COLUMN_GRANT_PUBLIC" --arg pgs "$COLUMN_PG_SCHEMA" \
		--arg tls "$(target_tls_mode)" --arg onfail "${DBMS_ON_FAILURE:-cleanup}" \
		--arg tdc "$TARGET_DB_CREATE" \
		'{csp:$csp, namePrefix:$prefix, nsId:$ns, provisioner:"cm-beetle",
		  migrator:"cm-centipede", collector:"cm-honeybee", srcMode:$sm, dstMode:"direct",
		  dstConnection:"beetleDb", targetDbCreate:$tdc,
	...
	if [ "${#CREATED_INSTANCES[@]}" -gt 0 ]; then
		if [ "${KEEP_INSTANCE:-0}" = "1" ]; then
			...
			for name in "${CREATED_INSTANCES[@]}"; do
				warn "  instance $name  (namespace $MATRIX_NS)"
			done
...
matrix_main() {
	...
	if [ "${CLEANUP_ONLY:-0}" = "1" ]; then
		banner "$CSP_TITLE — reclaiming what this matrix left in namespace $MATRIX_NS"
		beetle_preflight
		cleanup_all "$CSP"
		trap - EXIT INT TERM
		rm -rf "${MATRIX_TMP:-/nonexistent}"
	...
	# Checked before the summary below rather than with the other asserts, because
	# the summary prints which mode is active and normalises nothing itself.
	assert_target_db_create || die "TARGET_DB_CREATE check failed. Nothing was created."

	banner "0. pre-flight — $CSP_TITLE managed-RDBMS version matrix"
	info "name prefix : $MATRIX_NAME_PREFIX   namespace: $MATRIX_NS"
	info "region/zones: $(csp_env "$CSP" REGION)   $(csp_env "$CSP" ZONE) / $(csp_env "$CSP" ZONE2)   connection: $(connection_name "$CSP")"
	info "provision   : beetle $BEETLE_URL   tumblebug $TUMBLEBUG_URL"
	info "collect/move: honeybee $HB_BASE   centipede $CP_BASE"
	info "engines     : $engines"
	info "              managed by this CSP: $(csp_engines "$CSP")   creatable by beetle: $(beetle_engines "$CSP")"
	for engine in $engines; do
	...
	info "target conn : beetleDb reference (nsId=$MATRIX_NS) — centipede resolves the address"
	info "              through cm-beetle, so its own centipede.beetle.endpoint must name the"
	info "              same beetle as BEETLE_URL above"
	if [ "$TARGET_DB_CREATE" = "matrix" ]; then
		info "target db   : TARGET_DB_CREATE=matrix — this script creates it, so centipede finds"
		info "              one it does not own and a failed cell is emptied rather than dropped"
	else
		info "target db   : TARGET_DB_CREATE=centipede — centipede creates it through beetle and"
		info "              owns it, so a failed cell drops it whole"
	fi
	info "target TLS  : TARGET_TLS_MODE=$(target_tls_mode)   (transx-ex's connection setting for the migration)"
	for engine in $engines; do
	...
	assert_managed_engines "$engines" || die "engine check failed. Nothing was created."

	# Both CSPs need a network here, so both need two zones - there is no
	# default-VPC shortcut on either.
	local missing=""
	...
	[ "$(csp_env "$CSP" ZONE)" != "$(csp_env "$CSP" ZONE2)" ] \
		|| die "$(upper "$CSP")_ZONE and _ZONE2 are the same. A managed RDBMS wants subnets in two zones.
       Nothing was created."
	...
	# The env file holds the managed instance's master password in plain text, and
	# the file mode is the whole protection - there is no vault on this path.
	if [ -f "$ENV_FILE" ]; then
		local mode=""
	...
	HB_SG_ID="$(hb_source_group "${HB_SOURCE_GROUP:-cpbdb-matrix}")" \
		|| die "could not obtain the honeybee SourceGroup. Nothing was created."
	info "honeybee SourceGroup: ${HB_SOURCE_GROUP:-cpbdb-matrix} ($HB_SG_ID)"
	...
	sub "3) target versions — checked against the cb-tumblebug catalogue"
	...
	sub "4) network — vNet with two subnets, plus a security group"
	# Both CSPs. There is no default-VPC shortcut on the beetle path, and a managed
	# RDBMS wants subnets in two zones on either CSP.
	NETWORK_READY=1
...
matrix_defaults() {
	...
	# POLL_INTERVAL is centipede's. beetle's async waits use RDBMS_POLL_INTERVAL:
	# a migration settles in seconds, a managed instance in tens of minutes, and one
	# value cannot serve both without either hammering beetle or dozing through a
	# finished migration.
	...
	# No ADMIN_DB here. The target database is created and dropped through
	# cm-beetle's own logical database API, so nothing in this folder ever connects
	# to the instance to do it - whichever side makes the call.
	#
	# Who makes it is this. centipede is the default: the target is named as a
	# beetleDb reference, and creating the database behind such a reference is
	# something centipede does for itself (core/migration EnsureTargetDatabase),
	# through the same beetle API this script would have used. Letting it do so is
	# what this folder is here to exercise.
	#
	# "matrix" creates it first instead. centipede then finds a database it did not
	# create, and that flag decides the undo path for a failed cell: a database
	# centipede owns is dropped whole, one it merely found is emptied by transx-ex's
	# rollback and kept. Both are real situations - an operator who provisions the
	# target ahead of time gets the second - so both are runnable.
	TARGET_DB_CREATE="$(lower "${TARGET_DB_CREATE:-centipede}")"
	...
	# There is no secure-transport switch and no PostgreSQL template setting on this
	# path. Both belonged to resources the matrix created itself; cm-beetle owns the
	# instance and its logical databases, and exposes neither a parameter group nor a
	# template argument.
	...
	# Creating a managed instance, and how often the async request is polled.
	RDBMS_TIMEOUT="${RDBMS_TIMEOUT:-2400}"
	RDBMS_POLL_INTERVAL="${RDBMS_POLL_INTERVAL:-15}"
	# 3, not 1: cb-tumblebug returns "deletion unconfirmed" while the CSP is still
	# releasing a resource, and NCP takes minutes over it.
