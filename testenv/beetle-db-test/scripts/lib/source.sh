...
#   target database create   -> cm-beetle's logical database API (lib/beetle.sh)
...
# Names and ports
# ---------------------------------------------------------------------------
# Names and ports stay clear of every other environment in this repo, because
# several being up at once is the normal case: another folder serves the
# docker-to-docker scenario while this one serves managed targets, and a single
# task can want both.
...
ensure_ssh_key() {
	...
	ssh-keygen -q -t rsa -b 2048 -N '' -C 'beetle-db-test' -f "$key" || return 1
...
# ---------------------------------------------------------------------------
# MongoDB has no CREATE DATABASE, so cm-beetle's logical database API has nothing
# to call for it - a database begins to exist on the first write. So this one case
# is dropped with the client inside the source container, which keeps mongosh off
# the host running the matrix.
#
# ⚠ Not reachable today: beetle cannot build a managed MongoDB on either CSP
#   (BEETLE_ENGINES_<CSP> in lib/beetle.sh). Kept for when it can.
#
...
mongodb_drop_target() {
	...
	for db in $dbs; do
		out="$(docker exec "$name" mongosh --quiet $tls \
			...
			-u "$RDBMS_USER" -p "$(db_password "$(lower "$CSP")")" \
