...
# It is beetle-db-test's common.sh with the DBMS-specific parts removed: there is
# no version comparison here (object storage has no versions to compare) and no
# TLS mode table (an S3 endpoint's transport is decided by the provider, not by a
# setting this folder owns).
...
#
# The S3 keys travel in honeybee's connection_info body, so they are masked
# before anything reaches the screen or the file.
#
...
API_MASK_FILTER='
  walk(if type == "object"
       then with_entries(if (.key | IN("user","password","private_key",
