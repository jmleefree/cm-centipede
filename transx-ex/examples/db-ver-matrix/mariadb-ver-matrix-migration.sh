#!/usr/bin/env bash
#
# mariadb-ver-matrix-migration.sh — MariaDB version matrix migration (transx-ex called directly)
#
# Runs every combination of the source version list x the target version list,
# one cell at a time.
#   e.g. source=[10.6 11.4], target=[10.6 11.4] -> 10.6->10.6, 10.6->11.4, 11.4->10.6, 11.4->11.4 (4 cells)
#
# What one cell does:
...
# are ssh, transx-ex pipes mariadb-dump -> mariadb without writing a dump file; every
...
#
# Usage:
#   ./transx-ex/examples/db-ver-matrix/mariadb-ver-matrix-migration.sh
#   ./mariadb-ver-matrix-migration.sh --src-versions "10.6" --dst-versions "10.6 11.4"
#   ./mariadb-ver-matrix-migration.sh --only 11.4:10.6 --skip-version-check
#   ./mariadb-ver-matrix-migration.sh --mode ssh
#   ./mariadb-ver-matrix-migration.sh --src-mode ssh --dst-mode direct   # mixed access
#   MODE=ssh ./mariadb-ver-matrix-migration.sh
#   SRC_MODE=ssh DST_MODE=direct ./mariadb-ver-matrix-migration.sh
#
...
# ---------------------------------------------------------------------------
ENGINE="mariadb"
ENGINE_TITLE="MariaDB"
IMAGE_REPO="mariadb"
SSH_IMAGE_REPO="transxex-vermatrix-mariadb-ssh"
INTERNAL_DB_PORT=3306
DB_ADMIN_USER="root"
SRC_CONTAINER="transxex-vermatrix-mariadb-src"
DST_CONTAINER="transxex-vermatrix-mariadb-dst"
...
SRC_VERSIONS="${MARIADB_SRC_VERSIONS:-10.6 10.11 11.4 11.8}"
DST_VERSIONS="${MARIADB_DST_VERSIONS:-10.6 10.11 11.4 11.8}"
...
SRC_CHARSET="${MARIADB_SRC_CHARSET-utf8mb4}"
SRC_COLLATION="${MARIADB_SRC_COLLATION-utf8mb4_general_ci}"
DST_CHARSET="${MARIADB_DST_CHARSET:-$SRC_CHARSET}"
DST_COLLATION="${MARIADB_DST_COLLATION:-$SRC_COLLATION}"
...
SRC_DB_PORT="${MARIADB_SRC_DB_PORT:-45306}"
DST_DB_PORT="${MARIADB_DST_DB_PORT:-45307}"
SRC_SSH_PORT="${MARIADB_SRC_SSH_PORT:-45230}"
DST_SSH_PORT="${MARIADB_DST_SSH_PORT:-45231}"
...
Options:
  ...
  --only "SRC:DST ..."         run only these cells (e.g. --only "10.6:11.4")
...
Supported versions (official images): 10.6  10.11  11.4  11.8
USAGE
}
...
# ssh-tunnel mode runs mariadb-dump/mariadb on the remote host.
ensure_image() {
...
wait_ready() {
	...
	while [ "$waited" -lt "$READY_TIMEOUT" ]; do
		if docker exec -e MYSQL_PWD="$DB_ROOT_PASS" "$name" \
			mariadb -h 127.0.0.1 -P "$INTERNAL_DB_PORT" -u"$DB_ADMIN_USER" --batch --silent \
...
create_database() {
	...
	out="$(docker exec -e MYSQL_PWD="$DB_ROOT_PASS" "$name" \
		mariadb -h 127.0.0.1 -P "$INTERNAL_DB_PORT" -u"$DB_ADMIN_USER" \
...
seed_source() {
	...
	docker exec -i -e MYSQL_PWD="$DB_ROOT_PASS" "$SRC_CONTAINER" \
		mariadb -u"$DB_ADMIN_USER" --default-character-set=utf8mb4 "$SRC_DB" <<'SEED_SQL' 2>&1 | sed 's/^/      /'
...
snapshot_schema() {
	...
	docker exec -e MYSQL_PWD="$DB_ROOT_PASS" "$name" \
		mariadb -u"$DB_ADMIN_USER" --batch --silent --default-character-set=utf8mb4 -e "
SELECT CONCAT(
...
snapshot_data() {
	local name="$1" db="$2"
	docker exec -e MYSQL_PWD="$DB_ROOT_PASS" "$name" \
		mariadb -u"$DB_ADMIN_USER" --batch --silent --default-character-set=utf8mb4 "$db" -e "
SELECT CONCAT(
