#!/usr/bin/env bash
#
# postgresql-ver-matrix-migration.sh — PostgreSQL version matrix migration (transx-ex called directly)
#
# Runs every combination of the source version list x the target version list,
# one cell at a time.
#   e.g. source=[14 16], target=[14 16] -> 14->14, 14->16, 16->14, 16->16 (4 cells)
#
# What one cell does:
...
#      function/procedure/trigger/sequence + UTF-8 verification rows)
...
# are ssh, transx-ex pipes pg_dump -> psql without writing a dump file; every
...
#
# Usage:
#   ./transx-ex/examples/db-ver-matrix/postgresql-ver-matrix-migration.sh
#   ./postgresql-ver-matrix-migration.sh --src-versions "14" --dst-versions "14 16"
#   ./postgresql-ver-matrix-migration.sh --only 16:14 --skip-version-check
#   ./postgresql-ver-matrix-migration.sh --mode ssh
#   ./postgresql-ver-matrix-migration.sh --src-mode ssh --dst-mode direct   # mixed access
#   MODE=ssh ./postgresql-ver-matrix-migration.sh
#   SRC_MODE=ssh DST_MODE=direct ./postgresql-ver-matrix-migration.sh
#
...
IMAGE_REPO="postgres"
SSH_IMAGE_REPO="transxex-vermatrix-postgresql-ssh"
INTERNAL_DB_PORT=5432
DB_ADMIN_USER="postgres"
SRC_CONTAINER="transxex-vermatrix-postgresql-src"
DST_CONTAINER="transxex-vermatrix-postgresql-dst"
...
SRC_VERSIONS="${POSTGRESQL_SRC_VERSIONS:-13 14 15 16 17}"
DST_VERSIONS="${POSTGRESQL_DST_VERSIONS:-13 14 15 16 17}"
...
# Database encoding and locale — applied when the source/target databases are created.
#   An empty target value falls back to the source value, so the target is built
#   to match the source. An empty value omits the clause and takes the cluster default.
SRC_CHARSET="${POSTGRESQL_SRC_ENCODING-UTF8}"
SRC_COLLATION="${POSTGRESQL_SRC_LOCALE-en_US.utf8}"
DST_CHARSET="${POSTGRESQL_DST_ENCODING:-$SRC_CHARSET}"
DST_COLLATION="${POSTGRESQL_DST_LOCALE:-$SRC_COLLATION}"

# Compare the encoding snapshots only when both sides were configured the
...
SRC_DB_PORT="${POSTGRESQL_SRC_DB_PORT:-45432}"
DST_DB_PORT="${POSTGRESQL_DST_DB_PORT:-45433}"
SRC_SSH_PORT="${POSTGRESQL_SRC_SSH_PORT:-45250}"
DST_SSH_PORT="${POSTGRESQL_DST_SSH_PORT:-45251}"
...
Options:
  ...
  --only "SRC:DST ..."         run only these cells (e.g. --only "14:16")
...
Supported versions (official images): 13  14  15  16  17
USAGE
}
...
# ssh-tunnel mode runs pg_dump/psql on the remote host.
ensure_image() {
...
start_container() {
	...
	# The database is not created by an image init variable: the encoding and locale
	# have to come from the env file, so create_database builds it after startup.
...
wait_ready() {
	...
	while [ "$waited" -lt "$READY_TIMEOUT" ]; do
		if docker exec -e PGPASSWORD="$DB_ROOT_PASS" "$name" \
			psql -h 127.0.0.1 -p "$INTERNAL_DB_PORT" -U "$DB_ADMIN_USER" -d postgres \
...
# create_database ROLE — create the database with the encoding and locale from the env file.
...
create_database() {
	local role="$1" name db encoding locale clause=""
	if [ "$role" = "src" ]; then
		name="$SRC_CONTAINER"; db="$SRC_DB"; encoding="$SRC_CHARSET"; locale="$SRC_COLLATION"
	else
		name="$DST_CONTAINER"; db="$DST_DB"; encoding="$DST_CHARSET"; locale="$DST_COLLATION"
	fi
	[ -n "$encoding" ] && clause=" ENCODING '$encoding'"
	[ -n "$locale" ]   && clause="$clause LC_COLLATE '$locale' LC_CTYPE '$locale'"
	# PostgreSQL refuses an encoding or locale that differs from template1, so build
	# the database from template0 whenever either was asked for explicitly.
	[ -n "$clause" ]   && clause="$clause TEMPLATE template0"

	local out
	out="$(docker exec -e PGPASSWORD="$DB_ROOT_PASS" "$name" \
		psql -h 127.0.0.1 -p "$INTERNAL_DB_PORT" -U "$DB_ADMIN_USER" -d postgres \
		-v ON_ERROR_STOP=1 -q -c "CREATE DATABASE \"$db\"$clause" 2>&1)"
	if [ $? -ne 0 ]; then
		fail "$role database creation failed: $db$clause"
		printf '      %s\n' "$out"
		return 1
	fi
	info "$role database created: $db${clause:- (cluster default)}"
}
...
#   4 tables (3 FKs) / 1 view / 2 functions / 1 procedure / 1 trigger / 4 sequences
#   + UTF-8 verification rows (Korean, Japanese, emoji)
# ---------------------------------------------------------------------------
seed_source() {
	docker exec -i -e PGPASSWORD="$DB_ROOT_PASS" "$SRC_CONTAINER" \
		psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -p "$INTERNAL_DB_PORT" \
		-U "$DB_ADMIN_USER" -d "$SRC_DB" -q <<'SEED_SQL' 2>&1 | sed 's/^/      /'
...
snapshot_schema() {
	local name="$1" db="$2"
	docker exec -e PGPASSWORD="$DB_ROOT_PASS" "$name" \
		psql -h 127.0.0.1 -p "$INTERNAL_DB_PORT" -U "$DB_ADMIN_USER" -d "$db" -Atc "
SELECT 'tables='    || (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
    ...
    || ' seqs='     || (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
                         WHERE n.nspname = 'public' AND c.relkind = 'S')
    || ' funcs='    || (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
                         WHERE n.nspname = 'public' AND p.prokind = 'f')
    || ' procs='    || (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
    ...
    || ' fks='      || (SELECT count(*) FROM pg_constraint con JOIN pg_namespace n ON n.oid = con.connamespace
                         WHERE n.nspname = 'public' AND con.contype = 'f')
" 2>/dev/null | tr -d '\r' | tr -d '\n'
}

# snapshot_charset — the database encoding and locale, plus how many columns carry a collation
snapshot_charset() {
	local name="$1" db="$2"
	docker exec -e PGPASSWORD="$DB_ROOT_PASS" "$name" \
		psql -h 127.0.0.1 -p "$INTERNAL_DB_PORT" -U "$DB_ADMIN_USER" -d "$db" -Atc "
SELECT 'db=' || pg_encoding_to_char(encoding) || '/' || datcollate || '/' || datctype
    || ' collated_columns=' || (SELECT count(*) FROM pg_attribute a
                                 JOIN pg_class c ON c.oid = a.attrelid
                                 JOIN pg_namespace n ON n.oid = c.relnamespace
                                WHERE n.nspname = 'public' AND c.relkind = 'r' AND a.attcollation <> 0)
...
snapshot_data() {
	...
	docker exec -e PGPASSWORD="$DB_ROOT_PASS" "$name" \
		psql -h 127.0.0.1 -p "$INTERNAL_DB_PORT" -U "$DB_ADMIN_USER" -d "$db" -Atc "
SELECT 'rows=' || (SELECT count(*) FROM customers) || '/' || (SELECT count(*) FROM products)
               || '/' || (SELECT count(*) FROM orders) || '/' || (SELECT count(*) FROM order_items)
    || ' utf8=' || (SELECT name FROM customers WHERE id = 4) || '|' || (SELECT name FROM products WHERE id = 5)
    || ' sum='  || (SELECT to_char(COALESCE(SUM(qty * unit_price), 0), 'FM9999999990.00') FROM order_items)
" 2>/dev/null | tr -d '\r' | tr -d '\n'
}
...
run_cell() {
	...
	sub "2) create the databases (encoding and locale from the env file)"
	...
	if [ "$src_schema" != "$dst_schema" ]; then
		...
		detail="encoding mismatch — src[$src_charset] dst[$dst_charset]"
	fi
	# A difference the operator asked for is intentional, so it is not compared.
	[ "$CHARSET_COMPARE" = "1" ] || warn "encoding comparison skipped — source($SRC_CHARSET/$SRC_COLLATION) != target($DST_CHARSET/$DST_COLLATION)"
...
info "encoding        : source ${SRC_CHARSET:-(default)}/${SRC_COLLATION:-(default)}  target ${DST_CHARSET:-(default)}/${DST_COLLATION:-(default)}"
