...
This example demonstrates data migration using Object Storage with the transx-ex library.

## Overview

The transx-ex library supports three Object Storage access methods:
...
| `config-minio-download-with-filter.json`  | S3 → Local | Download via MinIO SDK, with a filter    |
| `config-spider-upload-with-filter.json`   | Local → S3 | Upload via CB-Spider, with a filter      |
...
| File                                 | Description                                                       |
| ------------------------------------ | ----------------------------------------------------------------- |
| `config-tumblebug-os2os-filter.json` | Transfer between Object Storages via Tumblebug, with a filter     |
...
./migrate.sh -c config-spider-upload-with-filter.json -v
```
...
```bash
./migrate.sh -c config-minio-download-with-filter.json -v
```
...
- `template-config-minio-download-with-filter.json`
- `template-config-spider-upload-with-filter.json`
