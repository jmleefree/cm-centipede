package mariadb

import (
	"context"
	"fmt"
	base "github.com/cloud-barista/cm-centipede/transx-ex/dbmsx/driver/base"
	filterpkg "github.com/cloud-barista/cm-centipede/transx-ex/dbmsx/filter"
	"os"
	"strings"
	"testing"
)

// =============================================================================
// Unit tests (no MariaDB server required)
// =============================================================================

func TestBuildMariaDBDumpCmd_ContainsMariaDBDump(t *testing.T) {
	drv := NewMariaDBDriver()
	cmd := drv.BuildDumpCmd(sshTunnelMariaDB(), base.ScopeFull)
	if !strings.HasPrefix(cmd, "mariadb-dump") {
		t.Errorf("BuildDumpCmd should start with 'mariadb-dump', got: %s", cmd)
	}
}

func TestBuildMariaDBDumpCmd_ContainsRequiredFlags(t *testing.T) {
	drv := NewMariaDBDriver()
	cmd := drv.BuildDumpCmd(sshTunnelMariaDB(), base.ScopeFull)
	for _, flag := range []string{"--routines", "--triggers", "--events", "--no-tablespaces"} {
		if !strings.Contains(cmd, flag) {
			t.Errorf("BuildDumpCmd missing %q: %s", flag, cmd)
		}
	}
}

func TestBuildMariaDBDumpCmd_SchemaOnly(t *testing.T) {
	drv := NewMariaDBDriver()
	cmd := drv.BuildDumpCmd(sshTunnelMariaDB(), base.ScopeSchemaOnly)
	if !strings.Contains(cmd, "--no-data") {
		t.Errorf("schema-only should have --no-data: %s", cmd)
	}
	if strings.Contains(cmd, "--no-create-info") {
		t.Errorf("schema-only must not have --no-create-info: %s", cmd)
	}
}

func TestBuildMariaDBDumpCmd_DataOnly(t *testing.T) {
	drv := NewMariaDBDriver()
	cmd := drv.BuildDumpCmd(sshTunnelMariaDB(), base.ScopeDataOnly)
	if !strings.Contains(cmd, "--no-create-info") {
		t.Errorf("data-only should have --no-create-info: %s", cmd)
	}
	if strings.Contains(cmd, "--no-data") {
		t.Errorf("data-only must not have --no-data: %s", cmd)
	}
}

func TestBuildMariaDBDumpCmd_DirectMode_ReturnsEmpty(t *testing.T) {
	drv := NewMariaDBDriver()
	loc := directMariaDB()
	if cmd := drv.BuildDumpCmd(loc, base.ScopeFull); cmd != "" {
		t.Errorf("BuildDumpCmd for direct mode should be empty, got %q", cmd)
	}
}

func TestBuildMariaDBRestoreCmd_ContainsMariaDB(t *testing.T) {
	drv := NewMariaDBDriver()
	cmd := drv.BuildRestoreCmd(sshTunnelMariaDB())
	if !strings.HasPrefix(cmd, "mariadb") {
		t.Errorf("BuildRestoreCmd should start with 'mariadb': %s", cmd)
	}
}

func TestBuildMariaDBRestoreCmd_NotMysql(t *testing.T) {
	drv := NewMariaDBDriver()
	cmd := drv.BuildRestoreCmd(sshTunnelMariaDB())
	if strings.HasPrefix(cmd, "mysql ") {
		t.Errorf("BuildRestoreCmd must not use 'mysql' (MariaDB uses 'mariadb'): %s", cmd)
	}
}

func TestBuildMariaDBRestoreCmd_DirectMode_ReturnsEmpty(t *testing.T) {
	drv := NewMariaDBDriver()
	if cmd := drv.BuildRestoreCmd(directMariaDB()); cmd != "" {
		t.Errorf("BuildRestoreCmd for direct mode should be empty, got %q", cmd)
	}
}

func TestMariadbExtractFKs_NoFK(t *testing.T) {
	createSQL := "CREATE TABLE `users` (\n  `id` int NOT NULL,\n  PRIMARY KEY (`id`)\n) ENGINE=InnoDB"
	clean, alters := mariadbExtractFKs("users", createSQL)
	if len(alters) != 0 {
		t.Errorf("expected no FK alters, got %v", alters)
	}
	if clean != createSQL {
		t.Errorf("clean SQL should be unchanged when no FKs present")
	}
}

func TestMariadbExtractFKs_WithFK(t *testing.T) {
	createSQL := "CREATE TABLE `orders` (\n" +
		"  `id` int NOT NULL,\n" +
		"  `user_id` int NOT NULL,\n" +
		"  PRIMARY KEY (`id`),\n" +
		"  CONSTRAINT `fk_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE\n" +
		") ENGINE=InnoDB"

	clean, alters := mariadbExtractFKs("orders", createSQL)

	if strings.Contains(clean, "FOREIGN KEY") {
		t.Errorf("clean SQL should not contain FOREIGN KEY:\n%s", clean)
	}
	if len(alters) != 1 {
		t.Fatalf("expected 1 ALTER, got %d: %v", len(alters), alters)
	}
	if !strings.Contains(alters[0], "ALTER TABLE `orders` ADD CONSTRAINT") {
		t.Errorf("alter statement format unexpected: %s", alters[0])
	}
	if strings.Contains(clean, "PRIMARY KEY (`id`),\n)") {
		t.Errorf("trailing comma not cleaned:\n%s", clean)
	}
}

func TestSplitMariaDBStatements_Simple(t *testing.T) {
	script := "CREATE TABLE t (id INT);\nINSERT INTO t VALUES (1);\n"
	stmts := splitMariaDBStatements(script)
	if len(stmts) != 2 {
		t.Fatalf("expected 2 statements, got %d: %v", len(stmts), stmts)
	}
	if !strings.HasPrefix(stmts[0], "CREATE TABLE") {
		t.Errorf("first stmt should be CREATE TABLE, got: %s", stmts[0])
	}
}

func TestSplitMariaDBStatements_DelimiterBlock(t *testing.T) {
	script := "CREATE TABLE t (id INT);\n" +
		"DELIMITER $$\n" +
		"CREATE PROCEDURE p()\n" +
		"BEGIN\n" +
		"  SELECT 1;\n" +
		"END$$\n" +
		"DELIMITER ;\n" +
		"INSERT INTO t VALUES (2);\n"

	stmts := splitMariaDBStatements(script)

	if len(stmts) != 3 {
		t.Fatalf("expected 3 statements, got %d: %v", len(stmts), stmts)
	}
	if !strings.Contains(stmts[1], "CREATE PROCEDURE") {
		t.Errorf("second stmt should be CREATE PROCEDURE, got: %s", stmts[1])
	}
	if !strings.Contains(stmts[1], "SELECT 1;") {
		t.Errorf("procedure body should be intact, got: %s", stmts[1])
	}
}

func TestSplitMariaDBStatements_EmptyInput(t *testing.T) {
	if stmts := splitMariaDBStatements(""); len(stmts) != 0 {
		t.Errorf("expected 0 statements for empty input, got %d", len(stmts))
	}
}

func TestMariadbFormatValue_Types(t *testing.T) {
	cases := []struct {
		input interface{}
		want  string
	}{
		{nil, "NULL"},
		{int64(42), "42"},
		{float64(3.14), "3.14"},
		{true, "1"},
		{false, "0"},
		{[]byte("hello"), "'hello'"},
		{"world", "'world'"},
		{"it's", `'it\'s'`},
	}
	for _, tc := range cases {
		got := mariadbFormatValue(tc.input)
		if got != tc.want {
			t.Errorf("mariadbFormatValue(%T(%v)) = %q, want %q", tc.input, tc.input, got, tc.want)
		}
	}
}

func TestMariadbBuildSelectQuery_NoExclusion(t *testing.T) {
	drv := NewMariaDBDriver()
	q, err := drv.buildSelectQuery(context.Background(), nil, "app", "users", nil)
	if err != nil {
		t.Fatalf("buildSelectQuery: %v", err)
	}
	if q != "SELECT * FROM `users`" {
		t.Errorf("unexpected query: %s", q)
	}
}

func TestMariadbDropColumns(t *testing.T) {
	in := "CREATE TABLE `users` (\n" +
		"  `id` int NOT NULL AUTO_INCREMENT,\n" +
		"  `password` varchar(255) DEFAULT NULL,\n" +
		"  `email` varchar(255) DEFAULT NULL,\n" +
		"  PRIMARY KEY (`id`),\n" +
		"  KEY `pw_idx` (`password`)\n" +
		") ENGINE=InnoDB"
	out := mariadbDropColumns(in, map[string]bool{"password": true})
	if strings.Contains(out, "`password`") || strings.Contains(out, "pw_idx") {
		t.Errorf("password column and its index should be dropped:\n%s", out)
	}
	if !strings.Contains(out, "`email`") || !strings.Contains(out, "PRIMARY KEY (`id`)") {
		t.Errorf("unrelated definitions must remain:\n%s", out)
	}
	if strings.Contains(out, ",\n)") {
		t.Errorf("dangling comma before closing paren:\n%s", out)
	}
}

func TestMariadbWithCollation(t *testing.T) {
	// The object has to be created under the source's collation_connection, and
	// the session value has to be put back afterwards so the next object in the
	// dump is not created under this one's.
	got := mariadbWithCollation("CREATE VIEW `v` AS SELECT 1;\n", "utf8mb4_general_ci")

	if !strings.Contains(got, "SET @saved_col_connection = @@collation_connection;") {
		t.Errorf("session collation not saved:\n%s", got)
	}
	// A string literal, not a back-quoted identifier: back quotes would make the
	// server read the value as a column reference.
	if !strings.Contains(got, "SET collation_connection = 'utf8mb4_general_ci';") {
		t.Errorf("collation not set as a string literal:\n%s", got)
	}
	if !strings.Contains(got, "SET collation_connection = @saved_col_connection;") {
		t.Errorf("session collation not restored:\n%s", got)
	}
	if before, _, _ := strings.Cut(got, "CREATE VIEW"); !strings.Contains(before, "SET collation_connection = 'utf8mb4_general_ci';") {
		t.Errorf("collation must be set before the statement, not after:\n%s", got)
	}
}

func TestMariadbWithCollation_DelimiterBlockStaysIntact(t *testing.T) {
	// The SET statements terminate with ";", so they must sit outside the
	// DELIMITER block — inside it the server would wait for "$$" instead.
	body := "DELIMITER $$\nCREATE PROCEDURE `p`() BEGIN SELECT 1; END$$\nDELIMITER ;\n"
	got := mariadbWithCollation(body, "utf8mb4_general_ci")

	setAt := strings.Index(got, "SET collation_connection = 'utf8mb4_general_ci';")
	blockAt := strings.Index(got, "DELIMITER $$")
	restoreAt := strings.Index(got, "SET collation_connection = @saved_col_connection;")
	endAt := strings.Index(got, "DELIMITER ;")
	if setAt < 0 || blockAt < 0 || restoreAt < 0 || endAt < 0 {
		t.Fatalf("wrapper is missing a part:\n%s", got)
	}
	if !(setAt < blockAt && endAt < restoreAt) {
		t.Errorf("SET statements must bracket the DELIMITER block, not sit inside it:\n%s", got)
	}
}

func TestMariadbWithCollation_EmptyIsNoOp(t *testing.T) {
	// SHOW CREATE TABLE reports no session context, and an older server may
	// report none either. Nothing to reproduce, so the statement is emitted bare
	// rather than wrapped around an empty value.
	stmt := "CREATE VIEW `v` AS SELECT 1;\n"
	if got := mariadbWithCollation(stmt, ""); got != stmt {
		t.Errorf("empty collation must return the statement unchanged:\n%s", got)
	}
}

func TestMariadbEnsureTableCollation(t *testing.T) {
	// SHOW CREATE TABLE left COLLATE= out because the collation is the default
	// for utf8mb4 on this server. Restoring that onto a server with a different
	// default would silently change it, so the catalog value is spliced in.
	in := "CREATE TABLE `t` (\n  `id` int NOT NULL\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
	got := mariadbEnsureTableCollation(in, "utf8mb4_general_ci")
	if !strings.Contains(got, "DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci") {
		t.Errorf("collation not spliced after the charset option:\n%s", got)
	}
}

func TestMariadbEnsureTableCollation_AlreadyPresent(t *testing.T) {
	// A statement that already names its collation must come back byte-identical
	// — a second COLLATE= would not parse.
	in := "CREATE TABLE `t` (\n  `id` int\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin"
	if got := mariadbEnsureTableCollation(in, "utf8mb4_general_ci"); got != in {
		t.Errorf("statement with an explicit collation must be unchanged:\n%s", got)
	}
}

func TestMariadbEnsureTableCollation_ColumnLevelClauseIsNotMistaken(t *testing.T) {
	// A column-level clause reads "COLLATE x" without the equals sign. Treating
	// it as the table option would leave the table itself unqualified.
	in := "CREATE TABLE `t` (\n" +
		"  `name` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin\n" +
		") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
	got := mariadbEnsureTableCollation(in, "utf8mb4_general_ci")
	if !strings.Contains(got, "DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci") {
		t.Errorf("column-level COLLATE suppressed the table option:\n%s", got)
	}
	if !strings.Contains(got, "COLLATE utf8mb4_bin") {
		t.Errorf("column-level COLLATE was altered:\n%s", got)
	}
}

func TestMariadbEnsureTableCollation_PartitionedTable(t *testing.T) {
	// The partition clause follows the table options. Appending to the end of the
	// statement would land inside it, so the splice goes right after the charset.
	in := "CREATE TABLE `t` (\n  `id` int NOT NULL\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4\n" +
		"/*!50100 PARTITION BY HASH (`id`)\nPARTITIONS 4 */"
	got := mariadbEnsureTableCollation(in, "utf8mb4_general_ci")
	if !strings.Contains(got, "DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci\n/*!50100 PARTITION BY") {
		t.Errorf("collation must sit before the partition clause:\n%s", got)
	}
}

func TestMariadbEnsureTableCollation_EmptyCollationIsNoOp(t *testing.T) {
	// The catalog had nothing to say — a table with no collation at all. Nothing
	// to splice, so the statement passes through.
	in := "CREATE TABLE `t` (\n  `id` int\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4"
	if got := mariadbEnsureTableCollation(in, ""); got != in {
		t.Errorf("empty collation must return the statement unchanged:\n%s", got)
	}
}

// The SSH path reads the mariadb CLI's tab-separated output by column position,
// so a column added to a shared query without widening the parser silently
// mis-assigns every field after it. The integration tests below need a live
// server and would not catch that; these do. Each row is written as
// mariadbQueryViaSSH hands it over — already unescaped, one element per column.

func TestMariadbParseTableRow(t *testing.T) {
	row := []string{
		"orders", "1200", "98304", "65536", "32768", "InnoDB",
		"utf8mb4_general_ci", "utf8mb4",
	}

	got, ok := mariadbParseTableRow(row)
	if !ok {
		t.Fatalf("mariadbParseTableRow rejected a full-width row: %v", row)
	}
	if got.Name != "orders" {
		t.Errorf("Name = %q, want %q", got.Name, "orders")
	}
	if got.RowCount != 1200 {
		t.Errorf("RowCount = %d, want 1200", got.RowCount)
	}
	if got.TotalSize != 98304 || got.DataSize != 65536 || got.IndexSize != 32768 {
		t.Errorf("sizes = total %d, data %d, index %d; want 98304/65536/32768",
			got.TotalSize, got.DataSize, got.IndexSize)
	}
	if got.Engine != "InnoDB" {
		t.Errorf("Engine = %q, want %q", got.Engine, "InnoDB")
	}
	// The two columns most at risk: they sit last, so an off-by-one swaps them
	// or leaves them empty rather than failing outright.
	if got.Collation != "utf8mb4_general_ci" {
		t.Errorf("Collation = %q, want %q", got.Collation, "utf8mb4_general_ci")
	}
	if got.CharacterSet != "utf8mb4" {
		t.Errorf("CharacterSet = %q, want %q", got.CharacterSet, "utf8mb4")
	}
}

func TestMariadbParseTableRow_ShortRowSkipped(t *testing.T) {
	// The pre-charset column list. Accepting it would report a table whose
	// collation and character set silently read empty.
	old := []string{"orders", "1200", "98304", "65536", "32768", "InnoDB"}
	if _, ok := mariadbParseTableRow(old); ok {
		t.Error("mariadbParseTableRow accepted a row missing the charset columns")
	}
}

func TestMariadbParseColumnRow(t *testing.T) {
	row := []string{
		"name", "varchar(100)", "YES", "", "MUL", "the display name",
		"utf8mb4", "utf8mb4_general_ci",
	}

	got, ok := mariadbParseColumnRow(row)
	if !ok {
		t.Fatalf("mariadbParseColumnRow rejected a full-width row: %v", row)
	}
	if got.Name != "name" || got.DataType != "varchar(100)" {
		t.Errorf("Name/DataType = %q/%q, want name/varchar(100)", got.Name, got.DataType)
	}
	if !got.IsNullable {
		t.Error("IsNullable = false, want true for is_nullable=YES")
	}
	if got.IsPrimary {
		t.Error("IsPrimary = true, want false for column_key=MUL")
	}
	if got.CharacterSet != "utf8mb4" {
		t.Errorf("CharacterSet = %q, want %q", got.CharacterSet, "utf8mb4")
	}
	if got.Collation != "utf8mb4_general_ci" {
		t.Errorf("Collation = %q, want %q", got.Collation, "utf8mb4_general_ci")
	}
}

func TestMariadbParseColumnRow_NonStringColumn(t *testing.T) {
	// A numeric column has no character set. mariadbColumnQuery coalesces both
	// catalog columns to '' so the CLI never writes the literal "NULL" into
	// them; this pins that the parser passes the empty strings through.
	row := []string{"id", "int", "NO", "", "PRI", "", "", ""}

	got, ok := mariadbParseColumnRow(row)
	if !ok {
		t.Fatalf("mariadbParseColumnRow rejected a full-width row: %v", row)
	}
	if !got.IsPrimary {
		t.Error("IsPrimary = false, want true for column_key=PRI")
	}
	if got.CharacterSet != "" || got.Collation != "" {
		t.Errorf("non-string column carried charset %q / collation %q, want both empty",
			got.CharacterSet, got.Collation)
	}
}

func TestMariadbParseColumnRow_ShortRowSkipped(t *testing.T) {
	old := []string{"name", "varchar(100)", "YES", "", "MUL", "the display name"}
	if _, ok := mariadbParseColumnRow(old); ok {
		t.Error("mariadbParseColumnRow accepted a row missing the charset columns")
	}
}

// TestMariadbSharedQueryColumnCount keeps the parsers and the queries in step: a
// parser reads eight columns, so each query must select eight. Counting
// top-level commas is enough because neither query nests a call with arguments.
func TestMariadbSharedQueryColumnCount(t *testing.T) {
	for _, tc := range []struct {
		name  string
		query string
	}{
		{"mariadbTableListQuery", mariadbTableListQuery},
		{"mariadbColumnQuery", mariadbColumnQuery},
	} {
		selectList := tc.query
		if i := strings.Index(selectList, " FROM "); i >= 0 {
			selectList = selectList[:i]
		}
		if n := strings.Count(selectList, ",") - strings.Count(selectList, "(") + 1; n != 8 {
			t.Errorf("%s selects %d columns, but the parser reads 8", tc.name, n)
		}
	}
}

// =============================================================================
// Integration tests (require MARIADB_TEST_DSN)
// =============================================================================
//
// Run mariadb_test.sh to execute all integration tests below automatically.
// The script handles container startup, fixture seeding, test execution, and cleanup.
//
// To run:
//
//	./mariadb_test.sh           # start container, run all TestMariaDB* tests, remove container on exit
//	./mariadb_test.sh --keep    # keep container running after tests (useful for debugging)
//
// Integration tests covered:
//
//	TestMariaDBTestConnection          — verify DB connection is reachable
//	TestMariaDBCheckTargetEmpty_EmptyDB — check whether the target database is empty
//	TestMariaDBDumpRestore_SchemaOnly   — produce a schema-only dump and validate the output file
//	TestMariaDBInspect                 — retrieve server version and database metadata
//
// To run manually without the script:
//
//	export MARIADB_TEST_DSN="root:pass@tcp(127.0.0.1:3307)/testdb_src?parseTime=true"
//	go test . -run TestMariaDB -v -timeout 120s

type mariadbTestEnv struct {
	loc    base.DBMSLocation
	srcDB  string
	destDB string
}

func mariadbTestSetup(t *testing.T) *mariadbTestEnv {
	t.Helper()
	if os.Getenv("MARIADB_TEST_DSN") == "" {
		t.Skip("MARIADB_TEST_DSN not set; skipping MariaDB integration tests")
	}

	loc := base.DBMSLocation{
		DBMSType:   base.DBMSTypeMariaDB,
		Database:   "testdb_src",
		AccessType: base.AccessTypeDirect,
		Direct:     &base.DirectConfig{Host: "localhost", Port: 3307, Username: "root", Password: "pass"},
	}
	if v := os.Getenv("MARIADB_TEST_HOST"); v != "" {
		loc.Direct.Host = v
	}
	if v := os.Getenv("MARIADB_TEST_USER"); v != "" {
		loc.Direct.Username = v
	}
	if v := os.Getenv("MARIADB_TEST_PASS"); v != "" {
		loc.Direct.Password = v
	}
	if v := os.Getenv("MARIADB_TEST_PORT"); v != "" {
		p := 0
		fmt.Sscanf(v, "%d", &p)
		if p > 0 {
			loc.Direct.Port = p
		}
	}

	drv := NewMariaDBDriver()
	if err := drv.TestConnection(context.Background(), loc); err != nil {
		t.Skipf("MariaDB not reachable: %v", err)
	}

	return &mariadbTestEnv{
		loc:    loc,
		srcDB:  "testdb_src",
		destDB: "testdb_dst",
	}
}

func TestMariaDBTestConnection(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()
	if err := drv.TestConnection(context.Background(), env.loc); err != nil {
		t.Fatalf("TestConnection failed: %v", err)
	}
}

func TestMariaDBCheckTargetEmpty_EmptyDB(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()
	loc := env.loc
	loc.Database = "information_schema"
	_ = drv.CheckTargetEmpty(context.Background(), loc)
	// information_schema returns non-empty; verify no panic.
}

func TestMariaDBDumpRestore_SchemaOnly(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()

	dir := t.TempDir()
	outputPath := dir + "/dump.sql"

	src := env.loc
	src.Database = env.srcDB

	if err := drv.Dump(context.Background(), src, base.ScopeSchemaOnly, outputPath, nil, nil); err != nil {
		t.Fatalf("Dump schema-only: %v", err)
	}

	content, err := os.ReadFile(outputPath)
	if err != nil {
		t.Fatalf("read dump: %v", err)
	}
	if len(content) == 0 {
		t.Fatal("dump file is empty")
	}
}

func TestMariaDBInspect(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()

	info, err := drv.Inspect(context.Background(), env.loc)
	if err != nil {
		t.Fatalf("Inspect: %v", err)
	}
	if info.ServerVersion == "" {
		t.Error("ServerVersion is empty")
	}
	if info.Database != env.loc.Database {
		t.Errorf("Database = %q, want %q", info.Database, env.loc.Database)
	}
}

// sshTunnelMariaDB returns a DBMSLocation with SSH tunnel access type for unit tests.
func sshTunnelMariaDB() base.DBMSLocation {
	return base.DBMSLocation{
		DBMSType:   base.DBMSTypeMariaDB,
		Database:   "testdb",
		AccessType: base.AccessTypeSSHTunnel,
		SSHTunnel: &base.SSHTunnelConfig{
			SSH:      &base.SSHConfig{Host: "ssh.example.com", Port: 22, Username: "deploy"},
			DBHost:   "127.0.0.1",
			DBPort:   3306,
			Username: "root",
			Password: "pass",
		},
	}
}

// directMariaDB returns a DBMSLocation with direct access type for unit tests.
func directMariaDB() base.DBMSLocation {
	return base.DBMSLocation{
		DBMSType:   base.DBMSTypeMariaDB,
		Database:   "testdb",
		AccessType: base.AccessTypeDirect,
		Direct:     &base.DirectConfig{Host: "localhost", Port: 3307, Username: "root", Password: "pass"},
	}
}

// =============================================================================
// Inspect metric integration tests (require MARIADB_TEST_DSN + seeded fixtures)
// =============================================================================

func TestMariaDBInspect_DefaultOmitsDetail(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()

	info, err := drv.Inspect(context.Background(), env.loc)
	if err != nil {
		t.Fatalf("Inspect: %v", err)
	}
	if len(info.Tables) == 0 {
		t.Fatal("expected non-empty table list")
	}
	for _, tbl := range info.Tables {
		if len(tbl.Columns) != 0 || len(tbl.Indexes) != 0 {
			t.Errorf("table %q: default Inspect must omit columns/indexes", tbl.Name)
		}
	}
	if info.Views != nil || info.Functions != nil || info.Procedures != nil ||
		info.Triggers != nil || info.Events != nil || info.ForeignKeys != nil {
		t.Errorf("default Inspect must omit schema-object lists, got %+v", info)
	}
}

func TestMariaDBInspect_ColumnsIndexes(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()

	loc := withMetric(env.loc, &base.MetricOption{MariaDB: &base.MariaDBMetric{
		Columns: bp(true), Indexes: bp(true),
	}})
	info, err := drv.Inspect(context.Background(), loc)
	if err != nil {
		t.Fatalf("Inspect: %v", err)
	}
	var users *base.TableInfo
	for i := range info.Tables {
		if info.Tables[i].Name == "users" {
			users = &info.Tables[i]
		}
	}
	if users == nil {
		t.Fatal("users table not found")
	}
	if len(users.Columns) == 0 || len(users.Indexes) == 0 {
		t.Errorf("expected columns and indexes for users, got %d cols %d idx",
			len(users.Columns), len(users.Indexes))
	}
	if info.Views != nil || info.Procedures != nil {
		t.Error("columns/indexes metric must not populate schema-object lists")
	}
}

func TestMariaDBInspect_SchemaObjects(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()

	loc := withMetric(env.loc, &base.MetricOption{MariaDB: &base.MariaDBMetric{
		Views: bp(true), Functions: bp(true), Procedures: bp(true),
		Triggers: bp(true), Events: bp(true), ForeignKeys: bp(true),
	}})
	info, err := drv.Inspect(context.Background(), loc)
	if err != nil {
		t.Fatalf("Inspect: %v", err)
	}
	if !containsAll(info.Views, "active_users") {
		t.Errorf("Views = %v, want active_users", info.Views)
	}
	if !containsAll(info.Functions, "user_count") {
		t.Errorf("Functions = %v, want user_count", info.Functions)
	}
	if !containsAll(info.Procedures, "add_user") {
		t.Errorf("Procedures = %v, want add_user", info.Procedures)
	}
	if !containsAll(info.Triggers, "trg_orders_ai") {
		t.Errorf("Triggers = %v, want trg_orders_ai", info.Triggers)
	}
	if !containsAll(info.Events, "ev_noop") {
		t.Errorf("Events = %v, want ev_noop", info.Events)
	}
	if !containsAll(info.ForeignKeys, "fk_orders_user") {
		t.Errorf("ForeignKeys = %v, want fk_orders_user", info.ForeignKeys)
	}
}

func TestMariaDBInspect_RowCountExact(t *testing.T) {
	env := mariadbTestSetup(t)
	drv := NewMariaDBDriver()

	loc := withMetric(env.loc, &base.MetricOption{MariaDB: &base.MariaDBMetric{RowCountExact: bp(true)}})
	info, err := drv.Inspect(context.Background(), loc)
	if err != nil {
		t.Fatalf("Inspect: %v", err)
	}
	for _, tbl := range info.Tables {
		if tbl.Name == "users" && tbl.RowCount != 2 {
			t.Errorf("users RowCount = %d, want 2 (exact)", tbl.RowCount)
		}
	}
}

func TestMariaDBRowExcludeClause(t *testing.T) {
	if got := mariadbRowExcludeClause(nil, "orders"); got != "" {
		t.Errorf("nil filter = %q, want empty", got)
	}
	f := &filterpkg.DBMSFilterOption{Rules: []filterpkg.FilterRule{
		filterpkg.ExcludeRows("orders", "a"),
		filterpkg.ExcludeRows("orders", "b"),
	}}
	got := mariadbRowExcludeClause(f, "orders")
	want := "(NOT (a) OR (a) IS NULL) AND (NOT (b) OR (b) IS NULL)"
	if got != want {
		t.Errorf("multi =\n  %q\nwant\n  %q", got, want)
	}
}

func TestMariadbConstraintName(t *testing.T) {
	got := mariadbConstraintName("ALTER TABLE `orders` ADD CONSTRAINT `fk_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)")
	if got != "fk_user" {
		t.Errorf("mariadbConstraintName = %q, want fk_user", got)
	}
	if got := mariadbConstraintName("ALTER TABLE `t` ADD PRIMARY KEY (`id`)"); got != "" {
		t.Errorf("no-constraint case = %q, want empty", got)
	}
}

func TestMariadbDropIndexes(t *testing.T) {
	in := "CREATE TABLE `orders` (\n" +
		"  `id` int NOT NULL,\n" +
		"  `created` datetime DEFAULT NULL,\n" +
		"  PRIMARY KEY (`id`),\n" +
		"  KEY `idx_created` (`created`)\n" +
		") ENGINE=InnoDB"
	f := &filterpkg.DBMSFilterOption{Rules: []filterpkg.FilterRule{
		filterpkg.ExcludeIndex("orders", "idx_created"),
	}}
	out := mariadbDropIndexes(in, "orders", f)
	if strings.Contains(out, "idx_created") {
		t.Errorf("idx_created should be dropped:\n%s", out)
	}
	if !strings.Contains(out, "PRIMARY KEY (`id`)") {
		t.Errorf("PRIMARY KEY must remain:\n%s", out)
	}
	if strings.Contains(out, ",\n)") {
		t.Errorf("dangling comma before closing paren:\n%s", out)
	}
}

// =============================================================================
// FK parent-side filtering (mariadbFKRef / mariadbFKRefDropped)
// =============================================================================

const fkAlterOrders = "ALTER TABLE `orders` ADD CONSTRAINT `fk_user` " +
	"FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE"

func TestMariadbFKRef(t *testing.T) {
	refTable, refCols := mariadbFKRef(fkAlterOrders)
	if refTable != "users" {
		t.Errorf("refTable = %q, want users", refTable)
	}
	if len(refCols) != 1 || refCols[0] != "id" {
		t.Errorf("refCols = %v, want [id]", refCols)
	}
}

func TestMariadbFKRef_Composite(t *testing.T) {
	alter := "ALTER TABLE `items` ADD CONSTRAINT `fk_ov` " +
		"FOREIGN KEY (`o_id`, `v_id`) REFERENCES `variants` (`o_id`, `v_id`)"
	refTable, refCols := mariadbFKRef(alter)
	if refTable != "variants" {
		t.Errorf("refTable = %q, want variants", refTable)
	}
	if len(refCols) != 2 || refCols[0] != "o_id" || refCols[1] != "v_id" {
		t.Errorf("refCols = %v, want [o_id v_id]", refCols)
	}
}

func TestMariadbFKRef_NoReferences(t *testing.T) {
	if refTable, refCols := mariadbFKRef("ALTER TABLE `t` ADD KEY `k` (`c`)"); refTable != "" || refCols != nil {
		t.Errorf("mariadbFKRef = (%q, %v), want empty", refTable, refCols)
	}
}

func TestMariadbFKRefDropped_NilFilter(t *testing.T) {
	if _, reason := mariadbFKRefDropped(fkAlterOrders, nil); reason != "" {
		t.Errorf("reason = %q, want empty for nil filter", reason)
	}
}

func TestMariadbFKRefDropped_ParentTableExcluded(t *testing.T) {
	f := &filterpkg.DBMSFilterOption{Rules: []filterpkg.FilterRule{
		filterpkg.ExcludeTable("users"),
	}}
	refTable, reason := mariadbFKRefDropped(fkAlterOrders, f)
	if refTable != "users" || reason != "referenced table excluded" {
		t.Errorf("got (%q, %q), want (users, referenced table excluded)", refTable, reason)
	}
}

func TestMariadbFKRefDropped_ParentColumnExcluded(t *testing.T) {
	f := &filterpkg.DBMSFilterOption{Rules: []filterpkg.FilterRule{
		filterpkg.ExcludeTable("users", "id"),
	}}
	_, reason := mariadbFKRefDropped(fkAlterOrders, f)
	if reason != "referenced column id excluded" {
		t.Errorf("reason = %q, want referenced column id excluded", reason)
	}
}

func TestMariadbFKRefDropped_UnrelatedExclusionKeepsFK(t *testing.T) {
	f := &filterpkg.DBMSFilterOption{Rules: []filterpkg.FilterRule{
		filterpkg.ExcludeTable("audit_log"),
		filterpkg.ExcludeTable("users", "password_hash"),
	}}
	if _, reason := mariadbFKRefDropped(fkAlterOrders, f); reason != "" {
		t.Errorf("reason = %q, want empty (FK must survive)", reason)
	}
}

// =============================================================================
// Unit tests for the SSH inspect transport (no MariaDB server required)
// =============================================================================

func TestMariadbUnescapeBatch(t *testing.T) {
	cases := []struct {
		name string
		in   string
		want string
	}{
		// The mariadb client escapes these in --batch mode; leaving them escaped
		// would corrupt multi-line column comments and default expressions.
		{"newline", `line1\nline2`, "line1\nline2"},
		{"tab", `a\tb`, "a\tb"},
		{"carriage return", `a\rb`, "a\rb"},
		{"backslash", `C:\\path`, `C:\path`},
		{"nul", `a\0b`, "a\x00b"},
		{"escaped backslash then t", `a\\tb`, `a\tb`},
		{"no escapes is returned as is", "plain value", "plain value"},
		{"trailing lone backslash", `abc\`, `abc\`},
		{"unknown escape kept verbatim", `a\qb`, `a\qb`},
		{"empty", "", ""},
	}
	for _, c := range cases {
		if got := mariadbUnescapeBatch(c.in); got != c.want {
			t.Errorf("%s: mariadbUnescapeBatch(%q) = %q, want %q", c.name, c.in, got, c.want)
		}
	}
}

func TestMariadbSSHMaxLine_ExceedsScannerDefault(t *testing.T) {
	// bufio.Scanner's default cap is 64 KiB; a long comment or view definition
	// would otherwise abort the query with "token too long".
	const scannerDefault = 64 * 1024
	if mariadbSSHMaxLine <= scannerDefault {
		t.Errorf("mariadbSSHMaxLine = %d, want more than the %d default", mariadbSSHMaxLine, scannerDefault)
	}
}

// =============================================================================
// CREATE DATABASE / DROP DATABASE statement building
// =============================================================================

func TestMariadbBuildCreateDatabase(t *testing.T) {
	cases := []struct {
		label string
		name  string
		opt   base.MariaDBCreateOption
		want  string
	}{
		{
			label: "no options",
			name:  "shop",
			want:  "CREATE DATABASE `shop`",
		},
		{
			label: "character set and collation",
			name:  "shop",
			opt:   base.MariaDBCreateOption{CharacterSet: "utf8mb4", Collate: "utf8mb4_general_ci"},
			want:  "CREATE DATABASE `shop` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci",
		},
		{
			label: "name needing quotes",
			name:  "shop 2024",
			want:  "CREATE DATABASE `shop 2024`",
		},
	}
	for _, c := range cases {
		t.Run(c.label, func(t *testing.T) {
			if got := mariadbBuildCreateDatabase(c.name, c.opt); got != c.want {
				t.Errorf("got  %q\nwant %q", got, c.want)
			}
		})
	}
}

func TestMariadbQuoteIdent(t *testing.T) {
	if got, want := mariadbQuoteIdent("shop"), "`shop`"; got != want {
		t.Errorf("got %q, want %q", got, want)
	}
	if got, want := mariadbQuoteIdent("a`b"), "`a``b`"; got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}

// =============================================================================
// Shared test helpers
// =============================================================================

// containsAll reports whether every want is present in got.
func containsAll(got []string, want ...string) bool {
	set := make(map[string]bool, len(got))
	for _, g := range got {
		set[g] = true
	}
	for _, w := range want {
		if !set[w] {
			return false
		}
	}
	return true
}

func withMetric(loc base.DBMSLocation, m *base.MetricOption) base.DBMSLocation {
	loc.Metric = m
	return loc
}

// =============================================================================
// Charset — contextually typed collations (MariaDB 11.4+)
// =============================================================================

func TestMariaDBExpandCollations_ContextuallyTyped(t *testing.T) {
	// MariaDB 11.4 lists the UCA-14.0.0 collations with no character set, and a
	// table collated with one reports the qualified name.
	rows := [][]string{
		{"utf8mb4_general_ci", "utf8mb4"},
		{"latin1_swedish_ci", "latin1"},
		{"uca1400_ai_ci", ""},
	}
	charsets := []string{"latin1", "ucs2", "utf8mb3", "utf8mb4"}

	got := make(map[string]bool)
	for _, n := range mariadbExpandCollations(rows, charsets) {
		got[n] = true
	}

	for _, want := range []string{
		"utf8mb4_general_ci", "latin1_swedish_ci", // kept as they come
		"uca1400_ai_ci",                                                        // the contextually typed name itself
		"utf8mb4_uca1400_ai_ci", "utf8mb3_uca1400_ai_ci", "ucs2_uca1400_ai_ci", // expanded forms
	} {
		if !got[want] {
			t.Errorf("expected %q in the expanded collation list", want)
		}
	}
	// A non-Unicode character set never carries a UCA collation, so none is invented.
	if got["latin1_uca1400_ai_ci"] {
		t.Error("latin1 must not gain a UCA collation name")
	}
}

func TestMariaDBExpandCollations_IgnoresBlankRows(t *testing.T) {
	got := mariadbExpandCollations([][]string{{}, {""}, {"  utf8mb4_bin  ", "utf8mb4"}}, []string{"utf8mb4"})
	if len(got) != 1 || got[0] != "utf8mb4_bin" {
		t.Errorf("got %v, want [utf8mb4_bin]", got)
	}
}

func TestMariaDBIsUnicodeCharset(t *testing.T) {
	cases := map[string]bool{
		"utf8mb4": true, "utf8mb3": true, "utf16": true, "utf16le": true, "utf32": true,
		"ucs2": true, "UCS2": true,
		"latin1": false, "binary": false, "euckr": false, "": false,
	}
	for in, want := range cases {
		if got := mariadbIsUnicodeCharset(in); got != want {
			t.Errorf("mariadbIsUnicodeCharset(%q) = %v, want %v", in, got, want)
		}
	}
}

// =============================================================================
// TLS
// =============================================================================

// TestMariaDBDSNTLSParam pins the mapping from a TLS mode onto the driver's
// tls= parameter. MariaDB speaks the MySQL wire protocol and is driven by the
// same Go package, so the mapping matches the MySQL driver's — and this test
// is what would catch the two drifting apart.
func TestMariaDBDSNTLSParam(t *testing.T) {
	tests := []struct {
		name       string
		cfg        base.DirectConfig
		want       string
		registered bool // a dbmsx- name rather than a built-in value
	}{
		{
			name: "an empty mode is disable",
			cfg:  base.DirectConfig{Host: "db"},
			want: "false",
		},
		{
			name: "disable",
			cfg:  base.DirectConfig{Host: "db", TLSMode: base.TLSModeDisable},
			want: "false",
		},
		{
			name: "prefer",
			cfg:  base.DirectConfig{Host: "db", TLSMode: base.TLSModePrefer},
			want: "preferred",
		},
		{
			name: "require",
			cfg:  base.DirectConfig{Host: "db", TLSMode: base.TLSModeRequire},
			want: "skip-verify",
		},
		{
			// The system trust store plus a hostname check is exactly tls=true,
			// so no configuration has to be registered for it.
			name: "verify-full without a CA is the driver's own tls=true",
			cfg:  base.DirectConfig{Host: "db", TLSMode: base.TLSModeVerifyFull},
			want: "true",
		},
		{
			// verify-ca has no built-in equivalent, so it registers whether or
			// not a CA was given.
			name:       "verify-ca registers a configuration",
			cfg:        base.DirectConfig{Host: "db", TLSMode: base.TLSModeVerifyCA},
			registered: true,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := dsnTLSParam(&tt.cfg)
			if err != nil {
				t.Fatalf("dsnTLSParam: %v", err)
			}
			if tt.registered {
				if !strings.HasPrefix(got, "dbmsx-") {
					t.Fatalf("tls=%q, want a registered dbmsx- name", got)
				}
				return
			}
			if got != tt.want {
				t.Errorf("tls=%q, want %q", got, tt.want)
			}
		})
	}
}

func TestMariaDBDSNTLSParam_RejectsAnUnreadableCA(t *testing.T) {
	// Reported here rather than as a connection failure much later.
	_, err := dsnTLSParam(&base.DirectConfig{
		Host: "db", TLSMode: base.TLSModeVerifyCA, TLSCAFile: "/nonexistent/ca.pem",
	})
	if err == nil {
		t.Fatal("expected an error for a CA file that is not there")
	}
}

func TestMariaDBBuildDSN_CarriesTheTLSParam(t *testing.T) {
	d := &MariaDBDriver{}
	dsn, err := d.buildDSN(base.DBMSLocation{
		DBMSType:   base.DBMSTypeMariaDB,
		AccessType: base.AccessTypeDirect,
		Database:   "app",
		Direct: &base.DirectConfig{
			Host: "db", Port: 3306, Username: "u", Password: "p",
			TLSMode: base.TLSModeRequire,
		},
	})
	if err != nil {
		t.Fatalf("buildDSN: %v", err)
	}
	if !strings.Contains(dsn, "tls=skip-verify") {
		t.Errorf("DSN does not carry the TLS parameter:\n%s", dsn)
	}
}
